...
export const GET = withTenantAuth(async (request, { tenantDb }) => {
  ...
  let sql = `SELECT * FROM service_addons WHERE is_active = true`;
  const params: any[] = [];
  
  if (service_id) {
    sql += ` AND service_id = $1::uuid`;
  ...
  try {
    const addons = await tenantDb.$queryRawUnsafe(sql, ...params);
...
export const POST = withTenantAuth(async (request, { tenantDb }) => {
  ...
  try {
    const newAddon = await tenantDb.$queryRawUnsafe(`
      INSERT INTO service_addons (service_id, name, description, price, duration_extra_minutes, is_required, display_order, is_active)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
      RETURNING *;
    `, 
      service_id, name, description || null, price || 0, duration_extra_minutes || 0, 
      is_required || false, display_order || 0, is_active !== false
    );

    return NextResponse.json({ success: true, addon: (newAddon as any)[0] });
  } catch (error: any) {

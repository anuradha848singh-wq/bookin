...
export function extractClinicSlug(host: string): string | null {
  ...
  if (hostWithoutPort.endsWith("localhost") || hostWithoutPort.endsWith("127.0.0.1")) {
    if (parts.length > 1 && parts[0] && parts[0] !== "localhost" && parts[0] !== "127") {
      clinicSlug = parts[0];
    }
  } else {
    ...
    const isVercelPreview = hostWithoutPort.includes("vercel.app") && parts.length > 2 && firstPart.includes("git");
    if (parts.length > 2 && !isVercelPreview && firstPart) {
      clinicSlug = firstPart;
    }
  }
